...
If `n` is even ( `n` being the number of gemstones ) we have made `(n-2)/2 * 4 = 2n - 4` queries else we have made `(n-3)/2 * 4  +  4 = 2n - 2` queries . Observing the prompt given in the `nc` connection it requires the player to make `2n - 2` queries . So if  `n` is even, we make two random queries at the end ( which won't affect our answer ) .
